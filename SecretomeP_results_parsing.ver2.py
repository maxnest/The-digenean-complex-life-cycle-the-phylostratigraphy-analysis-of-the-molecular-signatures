...
def secretomep_parsing(table, names_dict, secretomep_dict):
    for line in table:
        first_el = line.strip().split(" ")[0]
        if first_el in names_dict.values():
            description = [el for el in line.strip().split(" ") if len(el) != 0]
            #   print(description)
            #   ['g27982', '\t0.687\t2.262\t', '0.005\t', '-']
            #   ['g27983', '\t0.857\t4.595\t', '0.009\t', 'signal', 'peptide', 'predicted', 'by', 'SignalP']
            name, nn, warning = description[0], float(description[1].split("\t")[1]), description[-1]
            secretomep_dict[name] = {"nn": nn, "warning": warning}
            #   print(secretomep_dict)
...
if __name__ == "__main__":
    ...
    secretomep_parsing(args.table, names_dict, secretomep_dict)
